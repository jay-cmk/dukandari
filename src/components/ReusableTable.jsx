...
//                         {column.render
//                           ? column.render(row) // ✅ Use custom render (e.g. clickable name)
//                           : row[column.key] || (row[column.key] === 0 ? 0 : "-")}
//                       </td>
//                     ))}
//                     {showActions && (
...
export const ReusableTable = ({
  ...
  showEdit = true,
  showDelete = true,
  extraComponent = null, // ✅ optional extra component (FileFormatSelector)
}) => {
  const {
    query,
    pageSize,
    ...
    <div className="bg-gray-50 p-2 md:p-3">
      ...
      <div className="mb-4 rounded-lg bg-white p-1 shadow-sm border border-gray-200">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-end">
          <div className="flex items-center gap-3">
            <select
              className="rounded-lg border border-gray-300 bg-white px-2 py-1 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 focus:outline-none transition-colors"
              value={pageSize}
              onChange={(e) => handlePageSizeChange(Number(e.target.value))}
            >
              {[10, 15, 20, 25, 50].map((n) => (
          ...
          <div className="flex flex-1 md:flex-none items-center gap-3">
            ...
            {extraComponent && <div className="ml-2">{extraComponent}</div>}
          </div>
      ...
      <div className="rounded-md bg-white shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-400 border-b p-2 border-gray-200">
              <tr>
                <th className="px-2 text-left font-semibold text-gray-900 uppercase tracking-wider">
                  #
                </th>
                {columns.map((column) => (
                  <th
                    key={column.key}
                    className="px-2 text-left font-semibold text-gray-900 uppercase tracking-wider cursor-pointer bg-gray-400"
                    onClick={() => handleSort(column.key)}
                    aria-label={getAriaLabel(column.key)}
                  >
                    <div className="flex items-center gap-1">
                    ...
                    {columns.map((column) => (
                      <td
                        ...
                        className={`px-2 py-1 text-gray-900 ${
                          column.key === "prefixValue" ? "font-mono" : ""
                        }`}
                      >
                        {column.render
                          ? column.render(row)
