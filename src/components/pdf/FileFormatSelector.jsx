...
const FileFormatSelector = ({ data = [] }) => {
  ...
  // Preprocess rows to convert React elements to text
  const getExportData = () => {
    return data.map((row) => {
      const newRow = {};
      Object.keys(row).forEach((key) => {
        ...
        if (React.isValidElement(value)) {
          ...
          if (typeof value.props.children === "string") {
            newRow[key] = value.props.children;
  ...
  const exportToExcel = (data) => {
    const processedData = getExportData();
  ...
  const exportToPDF = (data) => {
    ...
    const tableColumn = Object.keys(processedData[0] || {});
