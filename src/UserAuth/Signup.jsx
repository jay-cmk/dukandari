...
// import { fetchCountries } from '../../src/redux/features/location/locationSlice';
...
//   // Fetch countries via Redux
//   useEffect(() => {
//     dispatch(fetchCountries());
//   }, [dispatch]);

//   // Default country setup when countries are loaded
//   useEffect(() => {
//     if (Array.isArray(countries) && countries.length > 0) {
//       const defaultCountry = countries[0];
//       setValue("country", defaultCountry.countriesCode);
//       setCountryDialCode(defaultCountry.countriesDialCode || '91');
//     }
//   }, [countries, setValue]);
...
//     const selectedCountry = countries.find(country => country.countriesCode === selectedCountryCode);
//     if (selectedCountry) {
//       setCountryDialCode(selectedCountry.countriesDialCode || '91');
//     }
//   };
...
//                         <option key={country.countriesCode} value={country.countriesCode}>
...
//                         <option 
//                           key={country.countriesCode} 
//                           value={country.countriesDialCode || '91'}
//                         >
//                           +{country.countriesDialCode || '91'}
//                         </option>
//                       ))}
//                     </select>
//                     <input
...
//                   <a
//                     href="https://www.vasyerp.com/terms-and-conditions.html"
//                     target="_blank"
...
//                   <a
//                     href="https://www.vasyerp.com/privacy-policy.html"
//                     target="_blank"
//                     rel="noopener noreferrer"
//                     className="text-blue-500 hover:text-blue-600"
//                   >
...
import { checkMobileThunk, verifyOtpThunk } from "../../src/redux/features/auth/authSlice";
import { sendOtpThunk } from "../../src/redux/features/auth/authSlice";
...
const Signup = () => {
  ...
  /*     */
  // Debounced function to check mobile number
  const debouncedCheckMobile = useCallback(
    debounce(async (number) => {
      if (number.length === 10) { // only check if number length is 10
        try {
          const result = await dispatch(checkMobileThunk(number)).unwrap();
          console.log("available", result.exists)
  ...
  const handlePhoneChangee = (e) => {
    const number = e.target.value;
    debouncedCheckMobile(number); // call API after debounce
  };
  ...
  // Check if OTP is complete
  useEffect(() => {
    const isOtpComplete = validateOtp(otp);
    setIsSubmitDisabled(!isOtpComplete);
  }, [otp]);
 ...
 const handleOtpChange = (index, value) => {
  ...
  if (digit !== '' && index < 5) { // ✅ 5 is the last index for 6 digits
    otpInputs.current[index + 1]?.focus();
  }
};
  ...
  const handleGetOtp = async (data) => {
    ...
    try {
      ...
      const otpResponse = await dispatch(
        sendOtpThunk({ mobileNo: data.phone }) // Key must match backend
      ).unwrap();
 ...
 const handleVerifyOtp = async () => {
  ...
  try {
    const result = await dispatch(
      verifyOtpThunk({ mobileNo: verifyOtpPhoneNo, otp: otpString })
    ).unwrap();
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center  border-amber-500 overflow-hidden">
      <div className="flex flex-row  gap-4 w-full min-h-screen bg-white shadow-lg overflow-hidden">
        {/* Left Side - Form */}
        <div className=" flex flex-col justify-between md:w-1/2 p-7 ">
          <div className="mb-4">
            <a href="https://www.vasyerp.com" target="_blank" rel="noopener noreferrer">
              <img src="./images/logo.png" className="h-12 mt-6" alt="VasyERP Logo" />
            </a>
          ...
          {!showOtpSection ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  ...
                  <Dropdown
                    ...
                    options={[
                      ...
                      ...(loading
                        ...
                        : error
                          ? [{ value: "error", label: "Error loading countries" }]
                          : countries.map((c) => ({
                            value: c.countriesCode,
                            label: c.countriesName,
                          }))
                      )
                    ]}
                    className="mb-3"
                ...
                <div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                    <div className={`h-2 rounded-full ${passwordStrengthColor}`} style={{ width: `${(passwordStrength / 5) * 100}%` }} ></div>
                  </div>
                  {errors.password && (
                    <p className="text-red-500 text-xs">{errors.password.message}</p>
                  )}
            ...
            <div className="space-y-6">
              ...
              <button
                type="button"
                onClick={handleVerifyOtp}
                className={`w-full py-2 px-4 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 font-medium ${isSubmitDisabled
                  ? 'bg-gray-400 cursor-not-allowed'
                  : 'bg-blue-500 text-white hover:bg-blue-600'
                  }`}
              >
